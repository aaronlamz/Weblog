  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (serviceId && templateId && publicKey) {
        ...
        // 备选方案：生成更友好的 mailto 链接
        const subject = encodeURIComponent(`Contact Form: ${data.subject}`)
        const body = encodeURIComponent(
          `Name: ${data.name}\nEmail: ${data.email}\n\nMessage:\n${data.message}`
        )
        const mailtoUrl = `mailto:${siteConfig.social.email}?subject=${subject}&body=${body}`
        
        window.open(mailtoUrl, '_blank')
        setStatus('success')
        setStatusMessage('已为您打开邮件客户端，请发送邮件。')
      }
    } catch (error) {
      console.error('Contact form error:', error)
      setStatus('error')
      setStatusMessage('发送失败，请直接发送邮件或稍后重试。')
    } finally {
